...
class ReportFormatter:
    ...
    def _display_description_results(self, results: Dict[str, Any], verbose: bool) -> None:
        ...
        if total > 0:
            ...
            stats_table.add_row("⚠️  Warnings", str(warnings), f"{(warnings/(warnings+errors+info_count) if (warnings+errors+info_count) > 0 else 0)*100:.1f}%")
            stats_table.add_row("❌ Errors", str(errors), f"{(errors/(warnings+errors+info_count) if (warnings+errors+info_count) > 0 else 0)*100:.1f}%")
            stats_table.add_row("ℹ️  Info", str(info_count), f"{(info_count/(warnings+errors+info_count) if (warnings+errors+info_count) > 0 else 0)*100:.1f}%")
        ...
        if issues:
            ...
            sorted_issues = sorted(issues, key=lambda x: (severity_order[x.severity], x.tool_name))
