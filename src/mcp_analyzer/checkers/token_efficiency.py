...
class TokenEfficiencyChecker:
    ...
    def __init__(self) -> None:
        ...
        self.pagination_params = [
            ...
            "cursor", "next_token", "continuation_token", "start", "count"
        ]
        ...
        self.format_control_params = [
            "format", "response_format", "detail_level", "verbosity",
            "compact", "full", "summary", "detailed"
        ]
    ...
    async def analyze_token_efficiency(
        ...
        for tool in tools:
            try:
                ...
                try:
                    ...
                    logger.warning(f"Failed to analyze tool {getattr(tool, 'name', 'unknown')}: {e}")
                    # Continue with static analysis only
                    
            except Exception as e:
                logger.error(f"Failed to analyze tool {getattr(tool, 'name', 'unknown')}: {e}")
                continue
        ...
        if tool_metrics:
            ...
            if all_measurements:
                token_counts = [m.token_count for m in all_measurements if m.token_count > 0]
                if token_counts:
                    stats["avg_tokens_per_response"] = int(sum(token_counts) / len(token_counts))
    ...
    async def _measure_response_sizes(self, tool: Any, mcp_client: Any) -> ResponseMetrics:
        ...
        for scenario in test_scenarios:
            try:
                ...
                measurements.append(ResponseMetric(
                    ...
                    has_verbose_identifiers=self._detect_verbose_identifiers(response),
                    is_truncated=self._detect_truncation(response)
                ))
                
                logger.debug(f"Tool {tool_name} scenario {scenario.name}: {token_count} tokens")
                ...
                logger.warning(f"Failed to execute {tool_name} with scenario {scenario.name}: {e}")
    ...
    def _analyze_response_metrics(self, metrics: ResponseMetrics) -> List[TokenEfficiencyIssue]:
        ...
        verbose_measurements = [m for m in metrics.measurements if m.has_verbose_identifiers]
        ...
        low_value_measurements = [m for m in metrics.measurements if m.contains_low_value_data]
    ...
    def _detect_low_value_data(self, response: Any) -> bool:
        ...
        low_value_count = sum(1 for pattern in low_value_patterns if re.search(pattern, response_str))
    ...
    def _detect_truncation(self, response: Any) -> bool:
        ...
        truncation_indicators = [
            "truncated", "more_available", "has_more", "continuation_token",
            "next_page", "partial", "limited", "excerpt"
        ]
    ...
    def _likely_returns_collections(self, tool: Any) -> bool:
        ...
        collection_indicators = [
            "list", "search", "find", "get_all", "fetch_all", "query",
            "browse", "index", "catalog", "directory", "collection"
        ]
    ...
    def _would_benefit_from_format_control(self, tool: Any) -> bool:
        ...
        detail_indicators = [
            "get", "fetch", "retrieve", "details", "info", "describe",
            "analyze", "report", "summary", "profile"
        ]
    ...
    def _generate_recommendations(self, issues: List[TokenEfficiencyIssue], stats: Dict[str, Any]) -> List[str]:
